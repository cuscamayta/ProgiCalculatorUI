...
describe('PriceCalculator.vue', () => {
  it('renders select with label and items', async () => {
    ...
    expect(wrapper.vm.$data.items).toEqual(['Common', 'Luxury']); // Asumiendo que items está en data
  });
  ...
  it('renders buttons with correct text', async () => {
    ...
    expect(buttons.length).toBe(1); // Ajustar según la cantidad de botones esperados
    expect(buttons[0].text()).toBe('Save Quote');
  });

  // Agregar más pruebas según sea necesario para otros elementos y funcionalidades
});
